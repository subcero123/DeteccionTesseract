...
def open_pdf(pdf_filename):
    return PyPDF2.PdfReader(open(pdf_filename, 'rb'))
...
def process_pdf(pdf_filename, nlp, output_filename):
    # Abre el archivo PDF
...
class ImprovedPDF(FPDF):
    def add_improved_text(self, paragraphs, nlp, interlineado=5):
        ...
        for paragraph in paragraphs:
            ...
            # Define colores para títulos y subtítulos
            if is_title:
                self.set_text_color(255, 0, 0)  # Rojo para títulos
            elif is_subtitle:
                self.set_text_color(0, 0, 255)  # Azul para subtítulos
...
def main(pdf_filename, output_filename):
    ...
    paragraphs = process_pdf(pdf_filename, nlp, output_filename)
    # Crea un objeto ImprovedPDF y genera el PDF mejorado
    pdf = ImprovedPDF()
    pdf.add_improved_text(paragraphs, nlp)
...
if __name__ == "__main__":
    ...
    pdf_filename = 'informacionOCR.pdf'
